 ...
 * how long this page waits for a test, and the page allows the station
...
/**
 * Where this charging station reads the time: its time servers, and the rules
 * for believing them.
 *
 ...
 * form told the station a lone host name, which the station read as a group of
 ...
 * what came of it.
 *
 * Every change takes effect at once, and the station is told the whole list
 * each time - which is why the list is only ever changed by exactly one server
 * at a time, from a dialog, and why a change the station refuses leaves the
 * page as it was.
 *
...
export const ntsPage: Page = {
    ...
    render({ root }) {
        ...
        /** The list as the station has it, as entries it can be told again. */
        ...
        /** The whole page, from what the station last said. */
        function draw(): void {
        ...
        /** Whether this station asks anybody at all. */
        function drawSwitch(): void {
            ...
            render(must<HTMLElement>(content, '#nts-switch'), html`
                ...
                <p class="hint">
                    Switched off, this station asks its time servers nothing at all - "Sync now" and the
         ...
         * last one the station remembers - and none at all while the next one
                    ...
                    <span class="hint">
                        ${mayTest
                              ? html`
                                    ...
                                    into the log. The clock of this station is not stepped by it.
                                `
                              : html`Asking the servers needs the CPO, the installer or the system administrator role.`}
                    </span>
                </div>
         ...
         *              station writes it into the log as it was sent.
         */
        ...
        async function switchTo(on: boolean): Promise<void> {
            ...
            {
                // Back to what the station has, which is not what the box
        ...
        /** Tell the station what the group is held to. */
...
/**
 * Milliseconds the way the station's log writes them: one place after a point,
