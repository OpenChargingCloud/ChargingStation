/**
 * What the web interface does when the station does not answer.
 *
 ...
 * requests left hanging by seven clicks, and a station that could not be
 * reached described to the operator in the browser's words rather than in
 * words about a charging station.
 */
...
/** A station that takes the request and then says nothing, ever. */
...
describe('a station that does not answer', () => {
    ...
    it('is given up on, rather than waited for as long as the browser is willing to', async () => {
        ...
        assert.ok(problem instanceof NoAnswer,
                  `a silent station produced ${problem} rather than a NoAnswer`);
    ...
    it('is given up on even when it is silent halfway through the body', async () => {
        ...
        // A station that sends its headers and then stops hangs exactly as
    ...
    it('is described in words about a charging station, not the browser\'s', async () => {

        // What the browser says here is "Failed to fetch", which names neither
        // the station nor anything to do about it.
        ...
        assert.match((problem as Error).message, /station/);
...
describe('what somebody is told when it runs out', () => {
    ...
    it('does not claim a write was not carried out, because nobody knows that', async () => {
        ...
        // The page stopped waiting; the station may well have written the file
...
describe('a station that does answer', () => {
    ...
    it('and says no is still an ApiError, with the station\'s own sentence', async () => {
...
describe('how long the page is willing to wait', () => {
    ...
    it('allows far more than the station has ever needed', () => {

        // Every read and write of this station's own configuration measured
    ...
    it('waits out the station\'s own patience when it has to ask somebody else', () => {

        // Measured: two name servers at three seconds each took 6.2 seconds,
        // because the station tries them in turn. A page that gave up at four
        // would be reporting its own impatience as the station's silence.
...
describe('signing in', () => {
    ...
    // The sign-in is the one call that does not go to this station's own API.
        ...
        fetchThat(() => {
            ...
            return Promise.resolve(
                call === 1
                    ...
                    : new Response(JSON.stringify({ username: 'root', roles: ['cpo'], permissions: ['changeAvailability'] }),
                                   { status: 200, headers: { 'Content-Type': 'application/json' } })
            );
        });
