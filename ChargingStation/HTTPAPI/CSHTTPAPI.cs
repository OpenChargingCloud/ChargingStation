...
namespace cloud.charging.open.ChargingStation
{
    ...
    /// <summary>
    /// The JSON API the browser talks to, registered at "/api": the
    /// configuration of this charging station, its log, and one Server-Sent
    /// Events stream that carries everything that happens.
    /// </summary>
    /// <remarks>
    ...
    /// through Hermod's MapEventSource. The sign-in itself is not here: it
    /// happens at Hermod's HTTPExt API, which is the only place that can check
    /// a password.
    /// </remarks>
    public class CSHTTPAPI : HTTPAPI
    {
        ...
        /// station cannot tell it from a station that has gone: nginx gives up
        ...
        /// membership carries this station's roles.
        /// </summary>
        ...
        /// rules are the NTS page's, and "legal" is decided by the station and
        /// sent as a fact, never worked out by whoever reads it.
        ///
        /// The same JSON the display gets inside its own answer, on its own
        /// port. A station started without a display served it nowhere, and a
        /// page of the administration could not ask it at all.
        /// </remarks>
        private Task<HTTPResponse> GetClock(HTTPRequest Request)
        {
            ...
            return Task.FromResult(
                       JSONResponse(Request, HTTPStatusCode.OK, Station.ClockJSON())
                   );
        ...
        private async Task<HTTPResponse> PostDNSQuery(HTTPRequest Request)
        {
            ...
            Log.Info($"'{user.Id}' asked this station to resolve '{name}'.", "dns", "test", "web");
        ...
        private async Task<HTTPResponse> PostNTSTest(HTTPRequest Request)
        {
            ...
            Log.Info($"'{user.Id}' asked this station to test {(host is null ? "its time server" : $"the time server '{host}'")}.",
                     "nts", "test", "web");
        ...
        /// </summary>
        /// <remarks>
        /// Two things in here are for a proxy in front of the station, and both
        ...
        /// not even its header - until a buffer is full or the station has been
        ...
        /// ask for its snapshot either. Measured behind nginx with the vehicle,
        /// whose stream is this one: the header came after 72 seconds, and the
        /// stream ended 98 ms later.
        ///
        /// And a comment whenever the stream has been silent for
        /// <see cref="EventStreamHeartbeat"/>, because the 60 seconds after
        /// which nginx gives up are an ordinary pause for a station nobody is
        /// using.
        /// </remarks>
        ...
        /// ever, one line of the log at a time. Among the few sessions a charging station has,
        ...
        /// <summary>
        /// Who is behind the request, when they are allowed to do this - or the
        ...
        /// from nobody signed in is a 401 that also expires a stale cookie, and a
