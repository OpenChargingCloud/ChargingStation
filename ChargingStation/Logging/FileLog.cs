...
namespace cloud.charging.open.ChargingStation.Logging
{
    ...
    /// nothing, and the ring buffer empties with the station. Everything that
    /// wants answering afterwards - what the time servers said at four in the
    ...
    /// the property that matters here: a station that is killed, or that
    /// crashes, has its last lines on disk rather than in a buffer.
    ///
    /// Nothing is ever deleted. A station that quietly threw away the evidence
    /// of the night somebody is asking about would be worse than one that needs
    /// a directory emptied now and then.
    ///
    /// A file that cannot be written does not take the station down, and does
    ...
    /// missing and since when. A station runs for weeks, so a disk that was
    /// full for an hour must not cost the rest of them - and a gap the file
    /// admits to is one somebody can reason about, where a silent one is only
    /// found by the person who needed what was in it.
    /// </remarks>
    public sealed class FileLog : IDisposable
    {
        ...
        public String FileFor(DateOnly Day)
            ...
            => Path.Combine(Directory, $"station-{Day:yyyy-MM-dd}.log");
