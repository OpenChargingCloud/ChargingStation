...
namespace cloud.charging.open.ChargingStation.Tests
{
    ...
    public class FileLogTests
    {
        ...
        {
            directory  = TestStations.TemporaryDirectory("file-log");
            clock      = new KioskTests.MovableClock(Afternoon);
        ...
        /// situation that matters: a station that is killed has its last lines
        /// on disk rather than in a buffer, and somebody following the file
        /// while the station runs sees each entry as it happens.
        /// </remarks>
        [Test]
        ...
        public void AFileIsADayAndTheDayIsUTC()
        {
            ...
            Assert.Multiple(() => {
                ...
                Assert.That(File.ReadAllLines(Path.Combine(directory, "station-2026-09-24.log")),
                            Is.EqualTo(new[] {
        ...
        /// <summary>
        /// A station started twice on one day adds to that day's file.
        /// </summary>
        [Test]
        ...
        public void AFileThatCannotBeWrittenIsSaidOnceAndItsGapWrittenDown()
        {
            ...
            var blocked  = Path.Combine(directory, "station-2026-09-23.log");
